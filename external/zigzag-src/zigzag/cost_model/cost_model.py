    ...
    * mapping: The combined spatial and temporal mapping object where access patterns are computed.
    ...
    def __init__(
        ...
        *,
        ...
        # --- DRAM-SRAM Compression: read config from top memory level ---
        self.compression_ratio: dict[MemoryOperand, float] = {}
        self.decompression_latency: dict[MemoryOperand, int] = {}
        for mem_op in core.memory_hierarchy.get_operands():
            top_mem_level = core.memory_hierarchy.get_operand_top_level(mem_op)
            compression_cfg = top_mem_level.memory_instance.compression
            if compression_cfg:
                op_str = str(mem_op)
                if op_str in compression_cfg:
                    self.compression_ratio[mem_op] = float(compression_cfg[op_str]["ratio"])
                    self.decompression_latency[mem_op] = int(
                        compression_cfg[op_str].get("decompression_latency", 0)
                    )
    ...
    def apply_dram_compression(self) -> None:
        """Scale DRAM-level memory word accesses by compression ratio for specified operands.
        This reduces the effective number of DRAM reads/writes, modeling compressed data storage
        in DRAM while keeping SRAM-side data volumes unchanged."""
        ...
        for layer_op in self.layer.layer_operands:
            ...
            top_mem_lv = self.mapping.mem_level[layer_op] - 1  # Highest level = DRAM
            if top_mem_lv < 0:
                continue

            original = self.memory_word_access[layer_op][top_mem_lv]
            # At the topmost (DRAM) level, data flows DOWN to SRAM:
            #   rd_out_to_low = DRAM reads out to SRAM (load) -> compress this
            #   wr_in_by_low  = SRAM writes back into DRAM (store) -> compress this
            # rd_out_to_high / wr_in_by_high are always 0 at top level (no higher level)
            self.memory_word_access[layer_op][top_mem_lv] = MemoryAccesses(
                ceil(original.rd_out_to_low / ratio),      # DRAM→SRAM load: compressed
                ceil(original.wr_in_by_low / ratio) if original.wr_in_by_low > 0 else 0,  # SRAM→DRAM store: compressed
                original.rd_out_to_high,                   # always 0 at top level
                original.wr_in_by_high,                    # always 0 at top level
            )
    ...
    def calc_allowed_and_real_data_transfer_cycle_per_data_transfer_link(self):
        ...
        for layer_op in self.layer.layer_operands:
            ...
            for mem_lv in range(self.mapping_int.mem_level[layer_op]):
                ...
                # --- DRAM-SRAM Compression: apply to top memory level transfer cycles ---
                is_top_mem_lv = (mem_lv == self.mapping_int.mem_level[layer_op] - 1)
                if is_top_mem_lv and mem_op in self.compression_ratio:
                    ratio = self.compression_ratio[mem_op]
                    decomp_lat = self.decompression_latency.get(mem_op, 0)
                    # At top level, data goes DOWN: rd_out_to_low = DRAM→SRAM, wr_in_by_low = SRAM→DRAM
